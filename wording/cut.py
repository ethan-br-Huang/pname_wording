...
class Cut:
    ...
    def cut(self, input_str, pretty_json=False):
        ...
        not_cht = regex.sub('[^a-zA-Z0-9-.]', ' ', input_str)

        eng_list = [i for i in not_cht.split(' ') if i]
        eng_list = [i.strip('-') for i in eng_list if i.strip('-')]

        seed_p1 = [w for w in eng_list if regex.match(
            '^(?=.*[0-9])(?=.*[a-z])([a-z0-9-]+)$', w)]
        seed_p2 = [w for w in eng_list if '-' in w]
        seed_p3 = [w for w in eng_list if regex.match('^[a-z]+-[a-z]+$', w)]
        ...
        for s in seed_list:
            if regex.match('^[0-9]+[a-z\-]+[0-9]+$', s):
                eng_dict['nash'].append(s)
            elif '-' in s:
                eng_dict['dash'].append(s)
            elif regex.match('^[0-9\.]+[a-z]+$', s):
                eng_dict['nash'].append(s)
            elif regex.match('^[a-z]+[0-9]+$', s):
